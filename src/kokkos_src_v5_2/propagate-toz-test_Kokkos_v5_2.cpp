...
#define N bsize
KOKKOS_FUNCTION void MultHelixPropEndcap(const MP6x6F_* A, const MP6x6SF_* B, MP6x6F_* C, const size_t n) {
...
KOKKOS_FUNCTION void MultHelixPropTranspEndcap(const MP6x6F_* A, const MP6x6F_* B, MP6x6SF_* C, const size_t n) {
  const float* a = A->data; //ASSUME_ALIGNED(a, 64);
  const float* b = B->data; //ASSUME_ALIGNED(b, 64);
  float* c = C->data;       //ASSUME_ALIGNED(c, 64);
  //Kokkos::parallel_for( Kokkos::TeamThreadRange(teamMember, bsize), [&] (const size_t n)
...
KOKKOS_FUNCTION void KalmanGainInv(const MP6x6SF_* A, const MP3x3SF_* B, MP3x3_* C, const size_t n) {
  const float* a = A->data; //ASSUME_ALIGNED(a, 64);
  const float* b = B->data; //ASSUME_ALIGNED(b, 64);
  float* c = C->data;       //ASSUME_ALIGNED(c, 64);
  //Kokkos::parallel_for( Kokkos::TeamThreadRange(teamMember,bsize), [&] (const size_t n)
...
KOKKOS_FUNCTION void KalmanGain(const MP6x6SF_* A, const MP3x3_* B, MP3x6_* C, const size_t n) {
  const float* a = A->data; //ASSUME_ALIGNED(a, 64);
  const float* b = B->data; //ASSUME_ALIGNED(b, 64);
  float* c = C->data;       //ASSUME_ALIGNED(c, 64);
  //Kokkos::parallel_for( Kokkos::TeamThreadRange(teamMember, bsize), [&](const size_t n)
...
KOKKOS_FUNCTION void KalmanUpdate(MP6x6SF_* trkErr, MP6F_* inPar, const MP3x3SF_* hitErr, const MP3F_* msP, const size_t it){
  MP3x3_ inverse_temp;
  MP3x6_ kGain;
  MP6x6SF_ newErr;
...
KOKKOS_FUNCTION void KalmanUpdate_v2(MP6x6SF_* trkErr, MP6F_* inPar, const MP3x3SF_* hitErr, const MP3F_* msP, const size_t it){
...
#pragma unroll
         for(size_t layer=0; layer<nlayer; ++layer) {
            ...
            propagateToZ(&(obtracks.cov), &(obtracks.par), &(obtracks.q), &(bhits.pos), &(obtracks.cov), &(obtracks.par), it); // vectorized function
            //KalmanUpdate(&(obtracks.cov),&(obtracks.par),&(bhits.cov),&(bhits.pos), it);
            KalmanUpdate_v2(&(obtracks.cov),&(obtracks.par),&(bhits.cov),&(bhits.pos), it);
         }
