...
__host__ __device__ size_t PosInMtrx(size_t i, size_t j, size_t D) {
  return i*D+j;
}
...
__host__ __device__ const MPTRK* bTk(const ALLTRKS* tracks, size_t ev, size_t ib) {
  return &((*tracks).btrks[ib + nb*ev]);
}

__host__ __device__ float q(const MP1I* bq, size_t it){
  return (*bq).data[it];
}
//
__host__ __device__ float par(const MP6F* bpars, size_t it, size_t ipar){
  return (*bpars).data[it + ipar*bsize];
}
__host__ __device__ float x    (const MP6F* bpars, size_t it){ return par(bpars, it, 0); }
__host__ __device__ float y    (const MP6F* bpars, size_t it){ return par(bpars, it, 1); }
__host__ __device__ float z    (const MP6F* bpars, size_t it){ return par(bpars, it, 2); }
__host__ __device__ float ipt  (const MP6F* bpars, size_t it){ return par(bpars, it, 3); }
__host__ __device__ float phi  (const MP6F* bpars, size_t it){ return par(bpars, it, 4); }
__host__ __device__ float theta(const MP6F* bpars, size_t it){ return par(bpars, it, 5); }

__host__ __device__ float par(const MPTRK* btracks, size_t it, size_t ipar){
  return par(&(*btracks).par,it,ipar);
}
__host__ __device__ float x    (const MPTRK* btracks, size_t it){ return par(btracks, it, 0); }
__host__ __device__ float y    (const MPTRK* btracks, size_t it){ return par(btracks, it, 1); }
__host__ __device__ float z    (const MPTRK* btracks, size_t it){ return par(btracks, it, 2); }
__host__ __device__ float ipt  (const MPTRK* btracks, size_t it){ return par(btracks, it, 3); }
__host__ __device__ float phi  (const MPTRK* btracks, size_t it){ return par(btracks, it, 4); }
__host__ __device__ float theta(const MPTRK* btracks, size_t it){ return par(btracks, it, 5); }
//
__host__ __device__ float par(const ALLTRKS* tracks, size_t ev, size_t tk, size_t ipar){
  size_t ib = tk/bsize;
  const MPTRK* btracks = bTk(tracks, ev, ib);
  size_t it = tk % bsize;
  return par(btracks, it, ipar);
}
__host__ __device__ float x    (const ALLTRKS* tracks, size_t ev, size_t tk){ return par(tracks, ev, tk, 0); }
__host__ __device__ float y    (const ALLTRKS* tracks, size_t ev, size_t tk){ return par(tracks, ev, tk, 1); }
__host__ __device__ float z    (const ALLTRKS* tracks, size_t ev, size_t tk){ return par(tracks, ev, tk, 2); }
__host__ __device__ float ipt  (const ALLTRKS* tracks, size_t ev, size_t tk){ return par(tracks, ev, tk, 3); }
__host__ __device__ float phi  (const ALLTRKS* tracks, size_t ev, size_t tk){ return par(tracks, ev, tk, 4); }
__host__ __device__ float theta(const ALLTRKS* tracks, size_t ev, size_t tk){ return par(tracks, ev, tk, 5); }
//
__host__ __device__ void setpar(MP6F* bpars, size_t it, size_t ipar, float val){
  (*bpars).data[it + ipar*bsize] = val;
}
__host__ __device__ void setx    (MP6F* bpars, size_t it, float val){ return setpar(bpars, it, 0, val); }
__host__ __device__ void sety    (MP6F* bpars, size_t it, float val){ return setpar(bpars, it, 1, val); }
__host__ __device__ void setz    (MP6F* bpars, size_t it, float val){ return setpar(bpars, it, 2, val); }
__host__ __device__ void setipt  (MP6F* bpars, size_t it, float val){ return setpar(bpars, it, 3, val); }
__host__ __device__ void setphi  (MP6F* bpars, size_t it, float val){ return setpar(bpars, it, 4, val); }
__host__ __device__ void settheta(MP6F* bpars, size_t it, float val){ return setpar(bpars, it, 5, val); }
//
__host__ __device__ void setpar(MPTRK* btracks, size_t it, size_t ipar, float val){
  return setpar(&(*btracks).par,it,ipar,val);
}
__host__ __device__ void setx    (MPTRK* btracks, size_t it, float val){ return setpar(btracks, it, 0, val); }
__host__ __device__ void sety    (MPTRK* btracks, size_t it, float val){ return setpar(btracks, it, 1, val); }
__host__ __device__ void setz    (MPTRK* btracks, size_t it, float val){ return setpar(btracks, it, 2, val); }
__host__ __device__ void setipt  (MPTRK* btracks, size_t it, float val){ return setpar(btracks, it, 3, val); }
__host__ __device__ void setphi  (MPTRK* btracks, size_t it, float val){ return setpar(btracks, it, 4, val); }
__host__ __device__ void settheta(MPTRK* btracks, size_t it, float val){ return setpar(btracks, it, 5, val); }

__host__ __device__ const MPHIT* bHit(const ALLHITS* hits, size_t ev, size_t ib) {
  return &((*hits).bhits[ib + nb*ev]);
}
//
__host__ __device__ float pos(const MP3F* hpos, size_t it, size_t ipar){
  return (*hpos).data[it + ipar*bsize];
}
__device__ float x(const MP3F* hpos, size_t it)    { return pos(hpos, it, 0); }
__device__ float y(const MP3F* hpos, size_t it)    { return pos(hpos, it, 1); }
__device__ float z(const MP3F* hpos, size_t it)    { return pos(hpos, it, 2); }
//
__host__ __device__ float pos(const MPHIT* hits, size_t it, size_t ipar){
  return pos(&(*hits).pos,it,ipar);
}
__device__ float x(const MPHIT* hits, size_t it)    { return pos(hits, it, 0); }
__device__ float y(const MPHIT* hits, size_t it)    { return pos(hits, it, 1); }
__device__ float z(const MPHIT* hits, size_t it)    { return pos(hits, it, 2); }
//
...
__global__ void prepareTracks(ATRK inputtrk, ALLTRKS* result,const float* trkrandos1,const float* trkrandos2, const float* randoq) {
...
__global__ void prepareHits(AHIT inputhit, ALLHITS *result,float* hitrandos1,float* hitrandos2) {
...
#define N bsize
__global__ void MultHelixPropEndcap(const MP6x6F* A, const MP6x6SF* B, MP6x6F* C) {
...
__global__ void MultHelixPropTranspEndcap(const MP6x6F* A, const MP6x6F* B, MP6x6SF* C) {
  const float* a = (*A).data; //ASSUME_ALIGNED(a, 64);
  const float* b = (*B).data; //ASSUME_ALIGNED(b, 64);
  float* c = (*C).data;       //ASSUME_ALIGNED(c, 64);
//#pragma omp simd
...
while(it<bsize){
 // printf("this is a test4: %d\n",it);
  ...
  MultHelixPropEndcap<<<32,32>>>(&errorProp, inErr, &temp);
  //cudaDeviceSynchronize();
  __syncthreads();
  MultHelixPropTranspEndcap<<<32,32>>>(&errorProp, &temp, outErr);
  __syncthreads();
  //cudaDeviceSynchronize();
 
}

__global__ void trackloop(const ALLTRKS* trk, const ALLHITS* hit, ALLTRKS* outtrk, int ie){
...
__global__ void eventloop(const ALLTRKS* trk, const ALLHITS* hit, ALLTRKS* outtrk){
//#pragma omp parallel for
	int ie = threadIdx.x + blockIdx.x*blockDim.x;
   //for (size_t ie=0;ie<nevts;++ie) { // loop over events
	while(ie<nevts){	
	trackloop<<<64,32>>>(trk,hit,outtrk,ie); 
	//cudaDeviceSynchronize();
	__syncthreads();
...
int main (int argc, char* argv[]) {
   ...
   int rsize = nevts*nb*bsize;
   float* trkrandos1;
   cudaMallocManaged((void**)&trkrandos1, 6*rsize*sizeof(float));
   float* trkrandos2;
   cudaMallocManaged((void**)&trkrandos2, 36*rsize*sizeof(float));
   float* hitrandos1;
   cudaMallocManaged((void**)&hitrandos1, 3*rsize*sizeof(float));
   float* hitrandos2;
   cudaMallocManaged((void**)&hitrandos2, 6*rsize*sizeof(float));
   float* randoq;
   cudaMallocManaged((void**)&randoq, 1*sizeof(float));
   randoq[0] = (float)rand() / RAND_MAX; 
  for (size_t ie=0;ie<nevts;++ie) {
    for (size_t ib=0;ib<nb;++ib) {
      for (size_t it=0;it<bsize;++it) {
	for (size_t ip=0;ip<36;++ip) {
		if( ip<6){
		hitrandos2[ie+ib*nevts+it*nb+ip*bsize]=randn(0,1);
		trkrandos1[ie+ib*nevts+it*nb+ip*bsize]=randn(0,1);
		if(ip<3){
		hitrandos1[ie+ib*nevts+it*nb+ip*bsize]=randn(0,1);
		}}
		trkrandos2[ie+ib*nevts+it*nb+ip*bsize]=randn(0,1);
//		//printf("rand: %f\n:",trkrandos[ie+ib*nevts+it*nb+ip*bsize]);
////  	  (*result).bhits[ib + nb*ie].pos.data[it + ip*bsize] = (1+smear*randn(0,1))*inputhit.pos[ip];
}
}
}
}
//cudaMemcpy(trkrandos_dev,trkrandos,rsize*sizeof(float),cudaMemcpyHostToDevice);
...
//    }
   ...
   //cudaProfilerStart();
   eventloop<<<512,512>>>(trk,hit,outtrk);
   cudaDeviceSynchronize();
   //__syncthreads
   //cudaProfilerStop();
   //cudaMemcpy(outtrk_dev,outtrk,sizeof(ALLTRKS),cudaMemcpyDeviceToHost);
...
//       //propagateToZ<<<128,128>>>(&(*btracks).cov, &(*btracks).par, &(*btracks).q, &(*bhits).pos, &(*obtracks).cov, &(*obtracks).par); // vectorized function
//	cudaDeviceSynchronize();
//    }
//  }
