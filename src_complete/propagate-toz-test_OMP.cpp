...
#pragma omp simd
  ...
  {
    //double det = 
    //  ((a[0*N+n]+b[0*N+n])*(((a[ 4*N+n]+b[ 4*N+n]) *(a[8*N+n]+b[8*N+n])) - ((a[7*N+n]+b[7*N+n]) *(a[5*N+n]+b[5*N+n])))) -
    //  ((a[1*N+n]+b[1*N+n])*(((a[ 1*N+n]+b[ 1*N+n]) *(a[8*N+n]+b[8*N+n])) - ((a[7*N+n]+b[7*N+n]) *(a[2*N+n]+b[2*N+n])))) +
    //  ((a[2*N+n]+b[2*N+n])*(((a[ 1*N+n]+b[ 1*N+n]) *(a[5*N+n]+b[5*N+n])) - ((a[2*N+n]+b[2*N+n]) *(a[4*N+n]+b[4*N+n]))));
    //double invdet = 1.0/det;

    //c[ 0*N+n] =  invdet*(((a[ 4*N+n]+b[ 4*N+n]) *(a[8*N+n]+b[8*N+n])) - ((a[7*N+n]+b[7*N+n]) *(a[5*N+n]+b[5*N+n])));
    //c[ 1*N+n] =  -1*invdet*(((a[ 3*N+n]+b[ 3*N+n]) *(a[8*N+n]+b[8*N+n])) - ((a[6*N+n]+b[6*N+n]) *(a[5*N+n]+b[5*N+n])));
    //c[ 2*N+n] =  invdet*(((a[ 3*N+n]+b[ 3*N+n]) *(a[7*N+n]+b[7*N+n])) - ((a[6*N+n]+b[6*N+n]) *(a[5*N+n]+b[5*N+n])));
    //c[ 3*N+n] =  -1*invdet*(((a[ 1*N+n]+b[ 1*N+n]) *(a[8*N+n]+b[8*N+n])) - ((a[7*N+n]+b[7*N+n]) *(a[2*N+n]+b[2*N+n])));
    //c[ 4*N+n] =  invdet*(((a[ 0*N+n]+b[ 0*N+n]) *(a[8*N+n]+b[8*N+n])) - ((a[6*N+n]+b[6*N+n]) *(a[2*N+n]+b[2*N+n])));
    //c[ 5*N+n] =  -1*invdet*(((a[ 0*N+n]+b[ 0*N+n]) *(a[7*N+n]+b[7*N+n])) - ((a[6*N+n]+b[6*N+n]) *(a[1*N+n]+b[1*N+n])));
    //c[ 6*N+n] =  invdet*(((a[ 1*N+n]+b[ 1*N+n]) *(a[5*N+n]+b[5*N+n])) - ((a[2*N+n]+b[2*N+n]) *(a[4*N+n]+b[4*N+n])));
    //c[ 7*N+n] =  -1*invdet*(((a[ 0*N+n]+b[ 0*N+n]) *(a[5*N+n]+b[5*N+n])) - ((a[2*N+n]+b[2*N+n]) *(a[3*N+n]+b[3*N+n])));
    //c[ 8*N+n] =  invdet*(((a[ 0*N+n]+b[ 0*N+n]) *(a[4*N+n]+b[4*N+n])) - ((a[1*N+n]+b[1*N+n]) *(a[3*N+n]+b[3*N+n])));
    //DOUBLE CHECK!!
    double det = 
