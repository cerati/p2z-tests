...
int main (int argc, char* argv[]) {
   ...
   for(itr=0; itr<NITER; itr++) {
     ...
     std::for_each(policy,
                   ...
                   auto ie = ii / nbxblk_sz;
                   auto ibt= ii - ie*nbxblk_sz;
                   auto ib = ibt / blk_sz;  
                   const int inner_loop_offset = ibt - ib*blk_sz;
...
//TESTTT
///////////
auto tmp_par_local2 = outtrkNacc.par[ii];//ib + nb*ie fails ??
...
#else
                     KalmanUpdate2<float, blk_sz>(outtrkNacc.par[ii], &inverse_temp, &kGain, &newErr);
                     //KalmanUpdate3<blk_sz>(outtrkNacc, ii, &inverse_temp, &kGain, &newErr);
