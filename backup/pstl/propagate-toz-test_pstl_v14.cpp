...
void propagateToZ_mod(const MPTRKAccessor &btracks, 
  ...
  const auto inErrPar = btracks.cov(lid);
...
//errorProp[0] = 2.2;
    ...
    //outPar[it] = 2.0;//MP6FAccessor::Get<iparX>(inPar, it) + k*(pxin*sina - pyin*(1.-cosa));
...
int main (int argc, char* argv[]) {
   ...
   for(itr=0; itr<NITER; itr++) {
     ...
     std::for_each(policy,
                   ...
                   const size_t inner_loop_offset = 0;//ibt - ib*blk_sz;
//const size_t block_sz_ = 128;
                   //const MPTRK* btracks = bTk(trk, ie, ib);
                   //MPTRK* obtracks      = bTk(outtrk, ie, ib);

#if 0
                   const size_t lid = ib + nb*ie;
auto tmp_par_local = outtrkNacc.par(ii);
...
#else
                     //propagateToZ_mod(trkNacc, hitNacc, *outtrkNaccPtr, *errorPropAccPtr, *tempAccPtr, ii, lii, inner_loop_offset);
#if 0
const auto inErrPar = trkNacc.cov(ii);
const auto inPar    = trkNacc.par(ii);

const auto inChg    = trkNacc.q(ii);
const auto msP      = hitNacc.pos(ii);

auto outErr = outtrkNacc.cov(ii);
auto outPar = outtrkNacc.par(ii);
outPar[0] = outPar[2]; 
auto errorProp = errorPropAcc();
//const float zout = msP[0];
const float zout = MP3FAccessor::Get<iparZ>(msP, 0);
const float k    = inChg[0]*kfact;//100/3.8;
const float deltaZ = zout - MP6FAccessor::Get<iparZ>(inPar, 0);
errorProp[2] = zout;

for (size_t it=inner_loop_offset;it<128; it += 1) {	
    const float zout = msP[it];
    errorProp[it] = zout;
}
#endif
...
//errorProp[0] = 2.0; 
  ...
  for (size_t it=inner_loop_offset;it<bsize; it += blk_sz) {	
//  for (size_t it=inner_loop_offset;it<block_sz; it += 1) {	
    const float zout = MP3FAccessor::Get<iparZ>(msP, it);
    const float k    = inChg[it]*kfact;//100/3.8;
    const float deltaZ = zout - MP6FAccessor::Get<iparZ>(inPar, it);
    const float pt   = 1. / MP6FAccessor::Get<iparIpt>(inPar, it);
