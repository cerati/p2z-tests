...
#pragma simd
  for (size_t it=offset; it<bsize; it += block_size) {
    ...
    newErr[0*bsize+it] = trkErr[0*bsize+it] - (kGain[0*bsize+it]*trkErr[0*bsize+it]+kGain[1*bsize+it]*trkErr[1*bsize+it]+kGain[2*bsize+it]*trkErr[2*bsize+it]);
    newErr[1*bsize+it] = trkErr[1*bsize+it] - (kGain[0*bsize+it]*trkErr[1*bsize+it]+kGain[1*bsize+it]*trkErr[6*bsize+it]+kGain[2*bsize+it]*trkErr[7*bsize+it]);
    newErr[2*bsize+it] = trkErr[2*bsize+it] - (kGain[0*bsize+it]*trkErr[2*bsize+it]+kGain[1*bsize+it]*trkErr[7*bsize+it]+kGain[2*bsize+it]*trkErr[11*bsize+it]);
     ...
     std::for_each(policy,
			...
			&newErrAcc = *newErrAccPtr ] (auto ii) {
                   ...
                   for(size_t layer=0; layer<nlayer; ++layer) {
                     ...
                     KalmanUpdate_mod<blk_sz>(outtrkNacc.cov, outtrkNacc.par, hitNacc, inverse_tempAcc, kGainAcc, newErrAcc, ii, lii, inner_loop_offset);
                   }
