...
struct MPNXAccessor {
   ...
   Get(size_t it, size_t id)    { return data_[id][it + ipar*bsize_]; }

   template <IPAR ipar, typename AccessedFieldTp = MPNTp> 
   typename std::enable_if<std::is_same<AccessedFieldTp, MP3F>::value, void>::type
   Set(size_t it, float val, size_t id)    { data_[id][it + ipar*bsize_] = val; }  
};
...
template <size_t block_size = 1>
void propagateToZ_mod(const MP6x6SF* inErr, const MP6F* inPar,
		  const MP1I* inChg, const MP3F* msP,
	                MP6x6SF* outErr, MP6F* outPar,
			MP6x6F* errorProp, MP6x6F* temp, const size_t offset = 0) {
  //
...
#ifdef __NVCOMPILER_CUDA__
                     const auto &bhits_pos = hitNacc.pos[layer+(ib + nb*ie)*nlayer];//?
                     const auto &bhits_cov = hitNacc.cov[layer+(ib + nb*ie)*nlayer];//?                   
#endif
                     //
#ifndef __NVCOMPILER_CUDA__
