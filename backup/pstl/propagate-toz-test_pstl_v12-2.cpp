...
struct MPNXAccessor {
   ...
   typename std::enable_if<(std::is_same<AccessedFieldTp, MP3F>::value and ipar < 3) or (std::is_same<AccessedFieldTp, MP6F>::value and ipar < 6), float>::type
   ...
   template <int ipar, typename AccessedFieldTp = MPNTp> 
   typename std::enable_if<std::is_same<AccessedFieldTp, MP3F>::value and ipar < 3, void>::type
   Set(size_t it, float val)    { data_[it + ipar*basesz] = val; }
   ...
   template <int ipar>
   static void Set(T* local_data, size_t it)     { local_data[it + ipar*basesz] = val; }  
...
void propagateToZ_mod(const MPTRKAccessor &btracks, 
		      ...
		      MP6x6FAccessor &errorProp, 
                      MP6x6FAccessor &temp, 
                      const size_t lid, 
                      const size_t llid, 
                      const size_t offset = 0) {

  const MP6x6SFAccessor &inErrAcc = btracks.cov;
  const MP6FAccessor    &inParAcc = btracks.par;

  const MP3FAccessor    &msPAcc   = bhits.pos;

  MP6x6SFAccessor &outErrAcc = obtracks.cov;
  MP6FAccessor    &outParAcc = obtracks.par;

  const auto inErrPar = obtracks.cov[lid];
  const auto inPar    = obtracks.par[lid];

  const auto inChg    = obtracks.q[lid];
  const auto msP      = bhits.pos[llid];

  auto outErr = btracks.cov[lid]; 
  auto outPar = btracks.par[lid]; 
  ...
  for (size_t it=offset;it<bsize; it += block_size) {	
    ...
    const float cCosPsina = cosf(cosP*sina);
#pragma unroll    
    for (size_t i=0;i<6;++i) errorProp.data_[bsize*PosInMtrx(i,i,6) + it] = 1.;
#endif
  }
...
int main (int argc, char* argv[]) {
   ...
   for(itr=0; itr<NITER; itr++) {
     ...
     std::unique_ptr<MP6x6FAccessor> errorPropAccPtr(new MP6x6FAccessor(errorProp));
     //MP6x6FAccessor &errorPropAcc = *errorPropAccPtr;

     MP6x6F &temp          = *tempPtr;
     std::unique_ptr<MP6x6FAccessor> tempAccPtr(new MP6x6FAccessor(temp));
     //MP6x6FAccessor &tempAcc = *tempAccPtr;

     MP3x3 &inverse_temp   = *inverse_tempPtr;
     std::unique_ptr<MP3x3Accessor> inverse_tempAccPtr(new MP3x3Accessor(inverse_temp));
     //MP3x3Accessor &inverse_tempAcc = *inverse_tempAccPtr;

     MP3x6 &kGain          = *kGainPtr;
     std::unique_ptr<MP3x6Accessor> kGainAccPtr(new MP3x6Accessor(kGain));
     //MP3x6Accessor &kGainAcc = *kGainAccPtr;

     MP6x6SF &newErr       = *newErrPtr;
     std::unique_ptr<MP6x6SFAccessor> newErrAccPtr(new MP6x6SFAccessor(newErr));
     //MP6x6SFAccessor &newErrAcc = *newErrAccPtr;
...
#ifdef __NVCOMPILER_CUDA__
                   const size_t lid = ib + nb*ie;

                   const auto &btracks_par = trkNacc.par[lid];
                   const auto &btracks_cov = trkNacc.cov[lid];
                   const auto &btracks_q   = trkNacc.q[lid];

                   const auto   &obtracks_par = outtrkNacc.par[lid];//?
                   const auto   &obtracks_cov = outtrkNacc.cov[lid];//?
                   const auto   &obtracks_q   = outtrkNacc.q[lid];//?
#endif
                  
                   for(size_t layer=0; layer<nlayer; ++layer) {
                     //const MPHIT* bhits = bHit(hit, ie, ib, layer);
#ifdef __NVCOMPILER_CUDA__
                     const size_t llid = layer+lid*nlayer;
...
#else
                     propagateToZ_mod(trkNacc, hitNacc, outtrkNacc, *errorPropAccPtr, *tempAccPtr, lid, llid, inner_loop_offset);
