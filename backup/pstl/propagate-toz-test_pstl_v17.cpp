...
#pragma simd
  for (size_t it=offset; it<bsize; it += block_size) {
    ...
    newErr01 = trkErr[0*bsize+it] - (kGain[0*bsize+it]*trkErr[0*bsize+it]+kGain[1*bsize+it]*trkErr[1*bsize+it]+kGain[2*bsize+it]*trkErr[2*bsize+it]);

    trkErr[0*bsize+it] = newErr01;

    newErr01 = trkErr[1*bsize+it] - (kGain[0*bsize+it]*trkErr[1*bsize+it]+kGain[1*bsize+it]*trkErr[6*bsize+it]+kGain[2*bsize+it]*trkErr[7*bsize+it]);
    newErr02 = trkErr[2*bsize+it] - (kGain[0*bsize+it]*trkErr[2*bsize+it]+kGain[1*bsize+it]*trkErr[7*bsize+it]+kGain[2*bsize+it]*trkErr[11*bsize+it]);
    newErr03 = trkErr[3*bsize+it] - (kGain[0*bsize+it]*trkErr[3*bsize+it]+kGain[1*bsize+it]*trkErr[8*bsize+it]+kGain[2*bsize+it]*trkErr[12*bsize+it]);
    newErr04 = trkErr[4*bsize+it] - (kGain[0*bsize+it]*trkErr[4*bsize+it]+kGain[1*bsize+it]*trkErr[9*bsize+it]+kGain[2*bsize+it]*trkErr[13*bsize+it]);
    newErr05 = trkErr[5*bsize+it] - (kGain[0*bsize+it]*trkErr[5*bsize+it]+kGain[1*bsize+it]*trkErr[10*bsize+it]+kGain[2*bsize+it]*trkErr[14*bsize+it]);
  
    newErr06 = trkErr[6*bsize+it] - (kGain[3*bsize+it]*trkErr[1*bsize+it]+kGain[4*bsize+it]*trkErr[6*bsize+it]+kGain[5*bsize+it]*trkErr[7*bsize+it]);

    trkErr[1*bsize+it] = newErr01;
    trkErr[6*bsize+it] = newErr06;

    newErr01 = trkErr[7*bsize+it] - (kGain[3*bsize+it]*trkErr[2*bsize+it]+kGain[4*bsize+it]*trkErr[7*bsize+it]+kGain[5*bsize+it]*trkErr[11*bsize+it]);
    newErr06 = trkErr[8*bsize+it] - (kGain[3*bsize+it]*trkErr[3*bsize+it]+kGain[4*bsize+it]*trkErr[8*bsize+it]+kGain[5*bsize+it]*trkErr[12*bsize+it]);
    newErr07 = trkErr[9*bsize+it] - (kGain[3*bsize+it]*trkErr[4*bsize+it]+kGain[4*bsize+it]*trkErr[9*bsize+it]+kGain[5*bsize+it]*trkErr[13*bsize+it]);
    newErr08 = trkErr[10*bsize+it] - (kGain[3*bsize+it]*trkErr[5*bsize+it]+kGain[4*bsize+it]*trkErr[10*bsize+it]+kGain[5*bsize+it]*trkErr[14*bsize+it]);
  
    newErr09 = trkErr[11*bsize+it] - (kGain[6*bsize+it]*trkErr[2*bsize+it]+kGain[7*bsize+it]*trkErr[7*bsize+it]+kGain[8*bsize+it]*trkErr[11*bsize+it]);

    trkErr[2*bsize+it]  = newErr02;
    trkErr[7*bsize+it]  = newErr01;
    trkErr[11*bsize+it] = newErr09;

    newErr02 = trkErr[12*bsize+it] - (kGain[6*bsize+it]*trkErr[3*bsize+it]+kGain[7*bsize+it]*trkErr[8*bsize+it]+kGain[8*bsize+it]*trkErr[12*bsize+it]);
    newErr01 = trkErr[13*bsize+it] - (kGain[6*bsize+it]*trkErr[4*bsize+it]+kGain[7*bsize+it]*trkErr[9*bsize+it]+kGain[8*bsize+it]*trkErr[13*bsize+it]);
    newErr09 = trkErr[14*bsize+it] - (kGain[6*bsize+it]*trkErr[5*bsize+it]+kGain[7*bsize+it]*trkErr[10*bsize+it]+kGain[8*bsize+it]*trkErr[14*bsize+it]);
    newErr10 = trkErr[15*bsize+it] - (kGain[9*bsize+it]*trkErr[3*bsize+it]+kGain[10*bsize+it]*trkErr[8*bsize+it]+kGain[11*bsize+it]*trkErr[12*bsize+it]);

    trkErr[3*bsize+it]  = newErr03;
    trkErr[8*bsize+it]  = newErr06;
    trkErr[12*bsize+it] = newErr02;
    trkErr[15*bsize+it] = newErr10;

    newErr03 = trkErr[16*bsize+it] - (kGain[9*bsize+it]*trkErr[4*bsize+it]+kGain[10*bsize+it]*trkErr[9*bsize+it]+kGain[11*bsize+it]*trkErr[13*bsize+it]);

    trkErr[16*bsize+it] = newErr03;

    newErr06 = trkErr[17*bsize+it] - (kGain[9*bsize+it]*trkErr[5*bsize+it]+kGain[10*bsize+it]*trkErr[10*bsize+it]+kGain[11*bsize+it]*trkErr[14*bsize+it]);

    trkErr[17*bsize+it] = newErr06;
  
    newErr02 = trkErr[18*bsize+it] - (kGain[12*bsize+it]*trkErr[4*bsize+it]+kGain[13*bsize+it]*trkErr[9*bsize+it]+kGain[14*bsize+it]*trkErr[13*bsize+it]);

    trkErr[4*bsize+it]  = newErr04;
    trkErr[9*bsize+it]  = newErr07;
    trkErr[13*bsize+it] = newErr01;
    trkErr[18*bsize+it] = newErr02;

    newErr10 = trkErr[19*bsize+it] - (kGain[12*bsize+it]*trkErr[5*bsize+it]+kGain[13*bsize+it]*trkErr[10*bsize+it]+kGain[14*bsize+it]*trkErr[14*bsize+it]);

    trkErr[19*bsize+it] = newErr10;

    newErr10 = trkErr[20*bsize+it] - (kGain[15*bsize+it]*trkErr[5*bsize+it]+kGain[16*bsize+it]*trkErr[10*bsize+it]+kGain[17*bsize+it]*trkErr[14*bsize+it]);

    trkErr[10*bsize+it] = newErr08;
    trkErr[5*bsize+it]  = newErr05;
    trkErr[14*bsize+it] = newErr09;
    trkErr[20*bsize+it] = newErr10;
     ...
     std::for_each(policy,
			...
			&kGainAcc = *kGainAccPtr] (auto ii) {
                   ...
                   for(size_t layer=0; layer<nlayer; ++layer) {
                     ...
                     KalmanUpdate_mod<blk_sz>(outtrkNacc.cov, outtrkNacc.par, hitNacc, inverse_tempAcc, kGainAcc, ii, lii, inner_loop_offset);
                   }
