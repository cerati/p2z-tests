...
struct MPNXAccessor {
   ...
   MPNXAccessor() : basesz(0), N(0), stride(0), data_(nullptr) {}
   ...
   template <IPAR ipar>
   static T Get(const T* local_data, size_t it)  { return local_data_[it + static_cast<int>(ipar)*basesz]; }  

   template <IPAR ipar>
   static void Set(T* local_data, size_t it)     { local_data_[it + static_cast<int>(ipar)*basesz] = val; }  
...
void propagateToZ_mod(const MPTRKAccessor &obtracks, const MP3FAccessor &msP, const size_t lid, const size_t llid, const size_t offset = 0) {
  ...
  const MP6x6SFAccessor &inErr = obtracks.cov;
  const MP6FAccessor    &inPar = obtracks.par;
  ...
  for (size_t it=offset;it<bsize; it += block_size) {	
    ...
    const float deltaZ = zout - inPar.Get<IPAR::Z>(it, lid);
    const float pt = 1./inPar.Get<IPAR::Ipt>(it, lid);
    const float cosP = cosf(inPar.Get<IPAR::Phi>(it, lid));
    const float sinP = sinf(inPar.Get<IPAR::Phi>(it, lid));
    const float cosT = cosf(inPar.Get<IPAR::Theta>(it, lid));
    const float sinT = sinf(inPar.Get<IPAR::Theta>(it, lid));
  }
