...
struct MPNXAccessor {
   ...
   template <IPAR ipar, typename AccessedFieldTp = MPNTp>
   typename std::enable_if<std::is_same<AccessedFieldTp, MP3F>::value or std::is_same<AccessedFieldTp, MP6F>::value, float>::type
   Get(size_t it, size_t id)  const { return (data_ + stride*id)[it + static_cast<int>(ipar)*basesz]; }

   template <IPAR ipar, typename AccessedFieldTp = MPNTp> 
   typename std::enable_if<std::is_same<AccessedFieldTp, MP3F>::value, void>::type
   Set(size_t it, float val, size_t id)    { (data_ + stride*id)[it + static_cast<int>(ipar)*basesz] = val; }

   // same as above but with a (shifted) raw pointer
   template <IPAR ipar>
   static T Get(const T* local_data, size_t it)  { return local_data[it + static_cast<int>(ipar)*basesz]; }  

   template <IPAR ipar>
   static void Set(T* local_data, size_t it)     { local_data[it + static_cast<int>(ipar)*basesz] = val; }  
...
template <typename Tp, size_t block_size = 1>
void KalmanUpdate2(Tp *trkErr, MP3x3* inverse_temp, MP3x6* kGain, MP6x6SF* newErr){
...
template <size_t block_size = 1>
void KalmanUpdate3(MPTRKAccessor &acc, const size_t id, MP3x3* inverse_temp, MP3x6* kGain, MP6x6SF* newErr){
...
template <size_t block_size = 1>
void propagateToZ_mod(const MPTRKAccessor &obtracks, const MP3FAccessor &msP, const size_t lid, const size_t llid, const size_t offset = 0) {

  const MP6x6SFAccessor &inErrAcc = obtracks.cov;
  const MP6FAccessor    &inParAcc = obtracks.par;

  const auto inPar = obtracks.cov[lid];
  const auto inChg = obtracks.q[lid];

  for (size_t it=offset;it<bsize; it += block_size) {	
    const float zout = msP.Get<IPAR::Z>(it, lid);
    const float k    = inChg[it]*kfact;//100/3.8;
    const float deltaZ = zout - MP6FAccessor::Get<IPAR::Z>(inPar, it);
    const float pt = 1./inParAcc.Get<IPAR::Ipt>(it, lid);
    const float cosP = cosf(inParAcc.Get<IPAR::Phi>(it, lid));
    const float sinP = sinf(inParAcc.Get<IPAR::Phi>(it, lid));
    const float cosT = cosf(inParAcc.Get<IPAR::Theta>(it, lid));
    const float sinT = sinf(inParAcc.Get<IPAR::Theta>(it, lid));
  }

}

void foo(MPTRKAccessor &in, const size_t local_id){
  auto   obtracks_par = in.par[local_id];//?
  const auto   obtracks_cov = in.cov[local_id];//?
  const auto   obtracks_q   = in.q[local_id];//?


  obtracks_par[0] = obtracks_par[2];
...
int main (int argc, char* argv[]) {
   ...
   std::unique_ptr<MPTRKAccessor> trkNaccPtr(new MPTRKAccessor(trkN));
   MPTRKAccessor &trkNacc = *trkNaccPtr;

   auto hitNPtr = prepareHitsN(inputhit);
   MPHIT &hitN = *hitNPtr;

   std::unique_ptr<MPHITAccessor> hitNaccPtr(new MPHITAccessor(hitN));
   MPHITAccessor &hitNacc = *hitNaccPtr;
...
#ifdef __NVCOMPILER_CUDA__
                   const size_t local_id = ib + nb*ie;

                   const auto &btracks_par = trkNacc.par[ib + nb*ie];
                   const auto &btracks_cov = trkNacc.cov[ib + nb*ie];
                   const auto &btracks_q   = trkNacc.q[ib + nb*ie];

                   const auto   &obtracks_par = outtrkNacc.par[local_id];//?
                   const auto   &obtracks_cov = outtrkNacc.cov[local_id];//?
                   const auto   &obtracks_q   = outtrkNacc.q[local_id];//?
#endif
                  
                   for(size_t layer=0; layer<nlayer; ++layer) {
                     //const MPHIT* bhits = bHit(hit, ie, ib, layer);
#ifdef __NVCOMPILER_CUDA__
                     const size_t llid = layer+local_id*nlayer;
                     auto bhits_pos = hitNacc.pos[llid];//?
                     auto bhits_cov = hitNacc.cov[llid];//?                   
#endif
                     //
#ifndef __NVCOMPILER_CUDA__
...
#else
                     //foo(outtrkNacc, local_id);
                     propagateToZ_mod(outtrkNacc, hitNacc.pos, local_id, llid, inner_loop_offset);
                     //propagateToZ_mod<blk_sz>(btracks_cov, btracks_par, btracks_q, bhits_pos, obtracks_cov, obtracks_par, &errorProp, &temp, inner_loop_offset); // vectorized function
                     //KalmanUpdate_mod<blk_sz>(obtracks_cov, obtracks_par, bhits_cov, bhits_pos, &inverse_temp, &kGain, &newErr, inner_loop_offset);
#endif
                   }
