...
void propagateToZ_mod(const MPTRKAccessor &obtracks, const MP3FAccessor &msP, const size_t lid, const size_t llid, const size_t offset = 0) {
  ...
  for (size_t it=offset;it<bsize; it += block_size) {	
    const float zout = msP.Get<iparZ>(it, lid);
    const float k    = inChg[it]*kfact;//100/3.8;
    const float deltaZ = zout - MP6FAccessor::Get<iparZ>(inPar, it);
    const float pt = 1./inParAcc.Get<iparIpt>(it, lid);
    const float cosP = cosf(inParAcc.Get<iparPhi>(it, lid));
    const float sinP = sinf(inParAcc.Get<iparPhi>(it, lid));
    const float cosT = cosf(inParAcc.Get<iparTheta>(it, lid));
    const float sinT = sinf(inParAcc.Get<iparTheta>(it, lid));
  }
