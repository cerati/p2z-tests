...
int main (int argc, char* argv[]) {
  ...
  using Host = alpaka::acc::AccCpuOmp2Threads<Dim, Idx>;
  ...
  WorkDiv const workDiv( blocksPerGrid, threadsPerBlock,elementsPerThread);
   ...
   //using ViewHost = alpaka::mem::buf::Buf<DevHost, MPTRK, Dim, Idx>
  ...
  //alpaka::mem::view::copy(queue,hit_dev,hit,nevts*nb*sizeof(MPHIT));
   ...
   for(itr=0; itr<NITER; itr++) {
     ...
     [] ALPAKA_FN_ACC (Host const & host, MPTRK* trk, MPHIT* hit, MPTRK* outtrk){
     alpaka_kernel(host, trk,hit,outtrk);
     }, trk, hit, outtrk);
...
//}
   ...
   printf("done ntracks=%i tot time=%f (s) time/trk=%e (s)\n", nevts*ntrks*NITER, (end-start)*0.001, (end-start)*0.001/(nevts*ntrks));
   printf("data region time=%f (s)\n", (end2-start2)*0.001);
   printf("memory transter time=%f (s)\n", ((end2-start2) - (end-start))*0.001);
   printf("setup time time=%f (s)\n", (setup_end-setup_start)*0.001);
   printf("formatted %i %f %f %f %f 0\n",nevts*ntrks*NITER, (end-start)*0.001, (end2-start2)*0.001,  ((end2-start2) - (end-start))*0.001, (setup_end-setup_start)*0.001);
